      ...
      <div class="top-bar-boxed flex items-center">
        ...
        >
          ...
          <span class="text-white text-lg ml-3">
            Help<span class="font-medium">ers</span>
          </span>
        </router-link>
        <!-- END: Logo -->
        <!-- BEGIN: Breadcrumb -->
        <div class="-intro-x breadcrumb breadcrumb--light mr-auto">
          ...
          <a href="" class="breadcrumb--active">Overview</a>
        </div>
        <!-- END: Breadcrumb -->
        <!-- BEGIN: Search -->
...
export default defineComponent({
  ...
  setup() {
    ...
    const menus = computed(() => nestedMenu(store.state.menu.menu, route))
    console.log()

    watch(
      computed(() => route.path),
      () => {
        formattedMenu.value = $h.toRaw(menus.value)
      }
    )
